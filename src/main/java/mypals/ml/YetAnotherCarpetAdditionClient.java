    ...
    public void onInitializeClient() {
        ...
        ClientTickEvents.END_CLIENT_TICK.register(client -> {
            while (carpetRulesKeyBind.wasPressed()) {
                MinecraftClient.getInstance().player.sendMessage(Text.literal("Requesting rules now！"),false);
        ...
        ClientPlayNetworking.registerGlobalReceiver(RulesPacketPayload.ID, (payload, context) -> {
            context.client().execute(() -> {
                ...
                context.client().player.sendMessage(Text.literal("Received " + chachedRules.size() + " rules from server！"),false);
                requesting = false;
                defaultRules.clear();
